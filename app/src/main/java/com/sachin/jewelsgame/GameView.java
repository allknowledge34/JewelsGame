...
public class GameView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public  void init(){
        ...
        for (int j=0;j<board.length;j++){
            topBoard[j] = new Jewel((int) (drawX+ j*cellWidth),(int) (drawX-cellWidth),0);
        }
    }
    public void update(){
        switch (gameState){
                ...
                if (board[i][j].color>0){
                    if (board[i+1][j].color == 0){
                        ...
                        if (((int)drawY + (i+1) * cellWidth)-board[i][j].poseY <cellWidth/8){
                            ...
                            board[i][j].poseX = (int) (drawX)+i *cellWidth;
                            drop_stop = true;
                        }
                    }
                }
    ...
    private boolean allCrushing(ArrayList<Point> points) {
        boolean allow = true;
